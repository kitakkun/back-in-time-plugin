...
sealed interface InspectorScreenEvent {
    ...
    data class BackInTime(val event: EventItemUiState) : InspectorScreenEvent
    ...
    EventEffect(eventEmitter) { event ->
        when (event) {
            is InspectorScreenEvent.TogglePropertyVisibility -> {
                pluginStateService.togglePropertyExpansion(event.instanceId)
            }

            is InspectorScreenEvent.SelectSession -> {
                pluginStateService.loadState(
                    pluginState.copy(globalState = pluginState.globalState.copy(selectedSessionId = event.id))
                )
            }
            ...
            is InspectorScreenEvent.SelectProperty -> {
                pluginStateService.loadState(
                    pluginState.copy(
                        inspectorState = pluginState.inspectorState.copy(
                            selectedInstanceId = event.instanceId,
                            selectedPropertyKey = event.name,
                        )
                    )
                )
            }
            ...
            is InspectorScreenEvent.UpdateHorizontalDividerPosition -> {
                pluginStateService.loadState(
                    pluginState.copy(inspectorState = pluginState.inspectorState.copy(horizontalSplitPanePosition = event.position))
                )
            }

            is InspectorScreenEvent.UpdateVerticalDividerPosition -> {
                pluginStateService.loadState(
                    pluginState.copy(inspectorState = pluginState.inspectorState.copy(verticalSplitPanePosition = event.position))
                )
            }

            is InspectorScreenEvent.SelectEvent -> {
                pluginStateService.loadState(
                    pluginState.copy(inspectorState = pluginState.inspectorState.copy(selectedEventId = event.event.id))
                )
            }

            is InspectorScreenEvent.BackInTime -> {
                // TODO
//                server.sendEvent(event.event.eventId)
            }
